...
//                        "0"+s
        ...
        } else if (id == R.id.btn_squ2) {
            if (str.charAt(0) == '-') {
                Toast.makeText(MainActivity.this, "负数不能", Toast.LENGTH_SHORT).show();
                text.setText("0");
    ...
    public static StringBuffer toPostfix(String infix){
        ...
        while(!stack.isEmpty())       //运算符出栈
            postfix.append(stack.pop());   //加到后缀表达式里
        return postfix;
    }
    ...
    private String getResult() {
        ...
        String re=String.format("%.7f",result);//误差
        re = re.replaceAll("0+?$", "");//去0
        re = re.replaceAll("[.]$", "");//若最后是.，则去掉
        return re;
    }

    private String isMatched(String infix) {
        Stack<String> stack=new Stack<String>();
